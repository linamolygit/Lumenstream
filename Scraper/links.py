...
class UniversalVideoExtractor:
    ...
    def extract_single_page(self, html: str, page_url: str) -> dict:
        ...
        # ---------- 1. Best source: window.initials.videoModel (xHamster etc) ----------
        ...
        if video_model and isinstance(video_model, dict):
            print("[*] Found videoModel in window.initials → using accurate trailer/sprite")

            if video_model.get("title"):
                data["title"] = video_model["title"]
            if video_model.get("duration"):
                data["duration"] = str(video_model["duration"])
            if video_model.get("views"):
                data["views"] = str(video_model["views"])

            # Correct preview for THIS video only
            ...
            if trailer and any(ext in trailer.lower() for ext in [".mp4", ".webm", ".mov"]):
                data["preview_videos"] = [trailer]

            # Correct sprite for THIS video only
            ...
            if isinstance(author, dict):
                data["channel"] = author.get("name") or author.get("username")
            ...
            if video_model.get("description"):
                data["description"] = video_model["description"]

            data["thumbnails"] = self._clean_list(data["thumbnails"])
            data["preview_videos"] = self._clean_list(data["preview_videos"])
            return data   # Early return – most accurate data

        # ---------- 2. Fallback: normal HTML parsing (limited scope) ----------
        # Title
        title = (
        ...
        # Thumbnails (main only)
        for meta in soup.select('meta[property="og:image"], meta[name="twitter:image"]'):
            if meta.get("content"):
                data["thumbnails"].append(meta["content"])
        data["thumbnails"] = self._clean_list(data["thumbnails"])

        # Sprite – only from main player area if possible
        ...
        # Preview videos – ONLY from main player container, NOT related videos
        # This is the key fix for wrong previews
        main_player = (
            soup.select_one("#video_box") or
            soup.select_one(".xplayer") or
            soup.select_one(".video-player") or
            soup.select_one("[data-role='xplayer']") or
        ...
        for attr in preview_attrs:
            for el in main_player.select(f"[{attr}]"):
                # Skip if this element is inside a related-videos / recommendations section
                parent_classes = " ".join(el.find_parent().get("class", []) if el.find_parent() else [])
                if any(x in parent_classes.lower() for x in ["related", "recommend", "suggest", "sidebar", "thumb-list"]):
                    continue
                val = el.get(attr)
    ...
    def extract_listing(self, html: str, base_url: str) -> list[dict]:
        ...
        # Different possible containers – more precise
        items = soup.select(
            ...
            'div[data-role="video-section-content-role"] > div'
        )
